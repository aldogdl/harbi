...
class Connections {
  ...
  Future<void> _ping(String tipo) async {
    ...
    if(tipo == 'reping') {
      // PrintScreen.accSingle('Recibiendo aviso de reconexión de: ${params['id']}');
    }
    ...
    if(inx != -1) {
      ...
      if(tipo == 'reping') {
        // PrintScreen.accSingleOk('¡Listo!, Reestableciendo CRON');
      }
    }else{
      if(tipo == 'reping') {
        // PrintScreen.accSingleErr('No se encotró ID:${params['id']}, entre las Conexiones.');
      }
    }
  }
